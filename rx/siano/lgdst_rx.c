...
#define USER_DATA_ENABLE                1  // to be disabled in echo mode, liyenho
#define CORRUPT_CHK_ENABLE              0
#define REMOTE_DATALOSS_CHECK           0  // to be enabled in echo mode, liyenho
...
static volatile uint8_t boot_and_app = 0;
static volatile uint8_t upgrade_boot_and_app=0;
static volatile uint8_t boot_only = 0;
static volatile uint8_t app_only = 0;
...
if(DEBUG_CTRLRX)
      ...
      //    (radio_rpacket[0]==0x73)&&(radio_rpacket[1]==0x73) )
      {
        static int pcnt=0;
      ...
      {// idle packet dataloss ----------------------------------
        ...
        if((radio_rpacket[0]==0xe5)&&(radio_rpacket[1]==0xe5)){
          for(i=7;i<RADIO_USR_RX_LEN;i++)
            if(radio_rpacket[i]!=0xe5) {
               byteerror++; becnt++;}
          if(CORRUPT_CHK_ENABLE)
      ...
      if(REMOTE_DATALOSS_CHECK)
      {// remote dataloss --------------------------------------
        //must turn off user data
        //got 0xe5 packet. the remote_rcv count should inc twice as fast as the remote_snd count
        int remote_rcv_err_curr=0, err_cnt;
        uint8_t byteval1, byteval2;
        static int remote_rcv_err_prev=0;
        byteval1 = radio_rpacket[5];
        byteval2 = radio_rpacket[6];
        if((radio_rpacket[0]==0xe5)&&(radio_rpacket[1]==0xe5)
           &&(byteval1==byteval2)){
          remote_rcv_err_curr = byteval1;
          err_cnt = fifolvlcalc(remote_rcv_err_curr, remote_rcv_err_prev, 256);
          if(err_cnt>5) err_cnt=5;
          for(i=0;i<err_cnt;i++){
            printf("                                                       ");
            printf("Remote packet loss = %d (%d,%d)",
                err_cnt, remote_rcv_err_curr,remote_rcv_err_prev);
            printf(" ^^^^^^^^^^^^^^^^^^^^^^^\n");}
          remote_rcv_err_prev = remote_rcv_err_curr;
        }
      } //0xe5 check
...
#endif
static void usb_open(int argc_sub, char **argv_sub)
{
  int  r = 1;
  	r = libusb_init(NULL);
	if (r < 0)
...
static int lgdst_upgrade_rx(int system_upgrade_v, int usbmsg_enable, char *upgrade_fwm_path_v)
{
...
static void usb_terminate(void)
{
  	//libusb_release_interface(devh,USB_DEV_INTF20);
	libusb_release_interface(devh,USB_DEV_INTF);
	if (detached) {
...
static void mutex_terminate(void){
     pthread_mutex_destroy(&mux);
}

static void bl_jump_to_app(void) //bootLoader command to jusp to application start address
{
		 libusb_control_transfer(devh, CTRL_OUT, USB_RQ,
...
{
	      printf("usage: sudo ./%s 0 servIP ctrlPort, bailed out...\n",argv[0]);
...
int lgdst_init_rx(int argc, char **argv)
{
	...
	do_exit = 1;
      if (3 > argc)  { // udp setup require ip addr
  ...
  //upgrade routines //////////////////////////////////////////////////////////////////
  if(system_upgrade) {
    r = lgdst_upgrade_rx(system_upgrade, 1, upgrade_fwm_path);
	usb_terminate();
  mutex_terminate();
	return(-2);
  }
  ...
  if(boot_and_app)
  {
	...
	bl_jump_to_app();
  sleep(3); //YH: must be 3 for ubuntu
  usb_terminate();
  sleep(3); //YH: must be 3 for ubentu
  usb_open(argc,argv);
  }
  if(upgrade_boot_and_app)
  {
  ...
  printf("ServIp =%s ctrlPrt=%d\n", servIP, ctrl_port_base);
