...
#endif
{
  ...
  int sz ;
  	  ...
  	  #endif
  	  	if (*size < frames_len) {
	  	  	puts("TS buffer is too small");
	  	  	*size = 0;
	  	  	return;
  	  	}
  	   *size = frames_len;
		if (!frames_len) return;
  	 #endif
 #endif
#endif // REC
	#ifdef REC
   	  ...
   	  ii = 188;
		  for (i=0; i<188; i++)
 	  	  ...
 	  	  pb0 += ii; // first TS block or non re-alignment needed
 	  		...
 	  		if (0x47 != tsbuf[188-chki]) {
 	  			pb0 = &tsbuf[188-chki]; // re-alignment failed @ 1st packet
			  ii = 188;
			  for (i=0; i<188; i++)
	 	  	  	 if (0x47 == pb0[i]) {
	 	  	 		ii = i;
   	...
   	while(0<=(r0-188)) {
   	  ...
   	  ii = 188;
		  for (i=0; i<188; i++)
 	  	...
 	  	// copy leftover
 	  	memcpy(chkbuf, tsbuf+(r-r0), r0);
 	  	 chki = r0;
   #endif
  #ifndef SRC_FRM_ENET // extract ts packets from socket
    #ifndef REALIGN_TS_PKT
      #if !defined(PES_FRM_PROT) && !defined(PES_FRM_PROT1)
			memcpy(tsbuf0, audbuf, r);
  #else
			memcpy(tsbuf0, retbuf, r);
		#endif
	  #else
			memcpy(tsbuf0, tsbuf, r1);
	  #endif
  #else
	int frag, sentsize;
