...
#define DEBUG 1
#define PACKET_LENGTH	15	// it shall be divisible into 6
...
int main(int argc, char *argv[])
{
  ...
  int ctrlrcv_socket, ctrlsnd_socket,err,rcvsize,sndcnt=0;  // for udp:
  ...
  rcvfifoempty = 0;
  while(!rcvfifoempty){
  ctrlsnd_tv.tv_sec = 0; // non-blocking recieve data
  ...
  /* data sent only when a packet was recieved (controlled within usb_test.c */
	  txbuffer[0] = 0xa5;
	  txbuffer[1] = 0xa5;
	  txbuffer[2] = ((unsigned char *)&sndcnt)[3];
	  txbuffer[3] = ((unsigned char *)&sndcnt)[2];
	  txbuffer[4] = ((unsigned char *)&sndcnt)[1];
	  txbuffer[5] = ((unsigned char *)&sndcnt)[0];
  ...
  printf("BaseStation: Sent: ");
  for(i=0;i<SNDBUFSIZE;i++) printf("%x ", (txbuffer[i])&0xff);
  printf("\n");
  usleep(500000); //define send/recieve speed @ 2 sec
  } //while(do_exit)
