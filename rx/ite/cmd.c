			...
			while(1) {
			 ...
			 if (0>acs->error ) {
				 switch(acs->error) {
					 case -128:
					 		not_ready += 1;
					 		printf("WARNING: Cmd_busTx: write not ready cnt=%d\n",
					 			not_ready);
					 		short_sleep (0.1); // extra wait to get atmel ready
					 		break;
					 case -127:
					 		got_nack += 1;
					 		printf("WARNING: Cmd_busTx: write nack cnt=%d\n",
					 			got_nack);
					 		error = Error_I2C_NACK_FAILED; // to be handled in standard.c module
					 		break;
					 default :
					 		mis_align += 1;
					 		printf("WARNING: Cmd_busTx: write misaligned cnt=%d\n",
					 			mis_align);
					 		goto exit;
				 }}
			 else { // transfer is ok
...
uint32_t Cmd_readRegisters (
   ...
   usleep(50000/*50000*/);  // gap write and read ops with sufficient delay // wangbin-0627
...
uint32_t Cmd_writeRegisters (
    ...
    usleep(10000/*50000*/);  // gap write and read ops with sufficient delay // wangbin-0627
