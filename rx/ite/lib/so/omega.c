...
unsigned long omega_init(
	...
	/*----- read -----*/
	error = omega_rmr(demodulator, chip, 0xEC86, 1, buf);		/*p_reg_p_tsm_init_clock_mode*/
	if (error) goto exit;
	error = omega_rmr(demodulator, chip, 0xED03, 1, &buf[1]);	/*p_reg_p_fbc_n_code*/
	...
	cnt = 1;
  puts("polling omega tuner init state, 1");
  ...
  puts("done, omega tuner init polling, 1");
	...
	if(chip_Type==0x9135 && chip_version == 2){
  	...
  	puts("done, omega tuner init polling, 2");
		if (buf[0] == 0) {
			error = Error_TUNER_INIT_FAIL;
	...
	/*----- write -----*/
	error = omega_wmr(demodulator, chip, 0xED81, 1, &val);		/*p_reg_p_iqik_m_cal*/

	if (error) goto exit;

	error = Standard_writeRegisterBits (demodulator, chip, Processor_LINK, p_reg_top_gpioh8_en, reg_top_gpioh8_en_pos, reg_top_gpioh8_en_len, 1);

	if (error) goto exit;
	error = Standard_writeRegisterBits (demodulator, chip, Processor_LINK, p_reg_top_gpioh8_on, reg_top_gpioh8_on_pos, reg_top_gpioh8_on_len, 1);

	if (error) goto exit;

exit:
	//printf("tuner init 0x%x\n",error);
	return (error);
}
unsigned long omega_setfreq(
	Demodulator*	demodulator,
	uint8_t			chip,
	unsigned int bw_kHz,
	...
	/*----- read -----*/
	error = omega_rmr(demodulator, chip, 0xEC4C, 1, buf);		/*p_reg_t_ctrl*/
	if (error) goto exit;
	error = omega_rmr(demodulator, chip, 0xED81, 1, &buf[1]);	/*p_reg_p_iqik_m_cal*/
	if (error) goto exit;
	...
	error = Standard_writeRegisters (demodulator, chip, Processor_OFDM, var_pre_lna_cap_sel, 1, val);
	if (error) goto exit;

	error = omega_wmr(demodulator, chip, 0xEC56, 1, &val[1]);	/*p_reg_t_lpf_bw*/
	if (error) goto exit;
	error = omega_wmr(demodulator, chip, 0xEC4C, 1, &val[2]);	/*p_reg_t_ctrl*/
	if (error) goto exit;
	/*----- the writing sequence is important for cal_freq and lo_freq, thus separate ----- */
	error = omega_wmr(demodulator, chip, 0xEC4D, 1, &val[3]);	/*p_reg_t_cal_freq_7_0*/
	if (error) goto exit;
	error = omega_wmr(demodulator, chip, 0xEC4E, 1, &val[4]);	/*p_reg_t_cal_freq_15_8*/
	if (error) goto exit;

	error = Standard_writeRegisters (demodulator, chip, Processor_OFDM, var_pre_lo_freq_7_0, 1, &val[5]);
	if (error) goto exit;

	error = Standard_writeRegisters (demodulator, chip, Processor_OFDM, var_pre_lo_freq_15_8, 1, &val[6]);

	if(rf_freq_kHz<c_fUHF_min)
		error = Standard_writeRegisterBits (demodulator, chip, Processor_LINK, p_reg_top_gpioh8_o, reg_top_gpioh8_o_pos, reg_top_gpioh8_o_len, 0);
	else
		error = Standard_writeRegisterBits (demodulator, chip, Processor_LINK, p_reg_top_gpioh8_o, reg_top_gpioh8_o_pos, reg_top_gpioh8_o_len, 1);
	if (error) goto exit;
exit:
	return (error);
}
...
unsigned long omega_rmr(
	...
	return (Standard_readRegisters (demodulator, chip, Processor_OFDM, mem_addr, len, buf));
}
/*omega_wmr (_write _multiple _registers)*/
unsigned long omega_wmr(
	Demodulator*	demodulator,
	uint8_t chip,
	unsigned long mem_addr,
	unsigned char len, unsigned char *val
) {
	return (Standard_writeRegisters (demodulator, chip, Processor_OFDM, mem_addr, len, val));
}
