...
//	printf("line # = %d\n", __LINE__);
 ...
 static void *ctrl_send_rx(void *arg) {
	#if USE_MAVLINK
		uint8_t *pb, tpacket[MAVLINK_USB_LEN] ;
	#else
	...
	#if USE_MAVLINK
			for (n=6; n<6+RADIO_USR_TX_LEN; n++) // 2 for chksum
	#else
...
static void *ctrl_recv_rx(void *arg) {
	#if USE_MAVLINK
		uint8_t *pb, rpacket[MAVLINK_USB_LEN] ;
	#else
	...
	#if USE_MAVLINK
			for (n=6; n<6+RADIO_USR_RX_LEN; n++) // 2 for chksum
	#else
