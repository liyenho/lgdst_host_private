...
#define true													1
#define false													0
#define print_usage                    puts("lgdst 0 rx [Vc]/Va/[Uc]/ns/s/wbs/ws/wb/w/rb/r/pair-id/pair-locked/Cst/MDst/temp/ctune/calib/calib-qry/hopless fpath/adr [bsz] [val0,val1,...], all numbers are in hex");
#define RAED_SETUP	\
...
bool update_ini(int mode,uint16_t *setting_ch,uint16_t *setting_att)
{
	FILE *fp;
	  ...
	  if (fp == NULL) {
		printf("ERROR: Fail to create 'usb_rx.ini' !!");
		return false;
	  }
	  if (mode) {// Tx
	    //if (setting_ch)
...
#define WRITE_SETUP(i)	\
   ...
   if (fast) { // only two registers written
   	WRITE_SETUP(0) // reset 6612 to take effect
   	WRITE_SETUP(2)
   	WRITE_SETUP(23)
	} else {
	   for (i=0; i<size; i++) {
...
#ifdef LIB
  int lgdst_access_rx_ctl(int argc,char **argv)
#else
