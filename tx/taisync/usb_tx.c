...
static void ctrl_poll_recv(void *arg) {
 ...
 #else // USE_MAVLINK
  uint8_t radio_rpacket1[MAVLINK_USB_LEN];
  MavLinkPacket newMav;
  bool complete = false ;
		pthread_mutex_lock(&mux);
			r = libusb_control_transfer(devh,
					CTRL_IN, USB_RQ,
					RADIO_COMM_VAL,
					RADIO_DATA_RX_IDX,
					radio_rpacket1, sizeof(radio_rpacket1), 0);
		pthread_mutex_unlock(&mux);
				if (r == sizeof(radio_rpacket1)) {
					for (i = 0; i < r; i++) {
						complete = Build_MavLink_from_Byte_Stream(&newMav, radio_rpacket1[i]);
						if (complete) {
...
static void ctrl_poll_send(void *arg) {  //usb send to module
 ...
 //#ifndef USE_MAVLINK, this signal path won't be used at the end, liyenho
		  pthread_mutex_lock(&mux);
...
printf("line # = %d\n", __LINE__);
#endif
#if defined(RADIO_TAISYNC) && defined(SRC_FRM_ENET)
...
//Setup Continuous process routines (threads) /////////////////////////////////////////
...
#else	 // Taisync platform bring up
	 dev_access *acs = (dev_access*)msg;

	acs->access = RDO_ANT;  // enable tx antenna
...
printf("line # = %d\n", __LINE__);
	...
	acs->access = OFDM_MCS;  // set modulation scheme
	acs->dcnt = sizeof(uint8_t);
	*((uint8_t*)acs->data) = BPSK_1B2;  // bpsk @ 1/2 c.r. @ 3 mb/s @ 10 mhz bw
printf("line # = %d\n", __LINE__);
		 	pthread_mutex_lock(&mux);
		r = libusb_control_transfer(devh,
						CTRL_OUT, USB_RQ,
...
printf("line # = %d\n", __LINE__);
        ...
        short_sleep (/*0.1*/0.01);
		 	...
		 	pthread_mutex_lock(&mux);
				libusb_control_transfer(devh,
							CTRL_OUT, USB_RQ,
							USB_STREAM_ON_VAL, //get atmel ready to stream
							USB_STREAM_IDX,
							NULL, 0, 0);
			pthread_mutex_unlock(&mux);
