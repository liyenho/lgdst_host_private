...
uint32_t IT9510Tuner_interpolation(
		...
		)
{
	...
	uint16_t TABLE_NROW = modulator->calibrationInfo.tableGroups;
...
uint32_t IT9510Tuner_setIQCalibration(
	...
	uint32_t   error = ModulatorError_NO_ERROR;
	uint32_t reg = 0;
	uint8_t c1_tmp_highbyte;
	uint8_t c1_tmp_lowbyte;
	uint8_t c2_tmp_highbyte;
	uint8_t c2_tmp_lowbyte;
	uint8_t c3_tmp_highbyte;
	uint8_t c3_tmp_lowbyte;
	uint8_t val[6];
	int dAmp = 0;
	int dPhi = 0;
	int* ptrdAmp = &dAmp;
	...
	val[0] = (uint8_t) c1_tmp_lowbyte;
	val[1] = (uint8_t) c1_tmp_highbyte;
	val[2] = (uint8_t) c2_tmp_lowbyte;
	val[3] = (uint8_t) c2_tmp_highbyte;
	val[4] = (uint8_t) c3_tmp_lowbyte;
	val[5] = (uint8_t) c3_tmp_highbyte;
exit:
